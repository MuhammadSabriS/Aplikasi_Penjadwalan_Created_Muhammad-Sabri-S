...
if(isset($_POST['submit']))
             ...
             $data = mysqli_query($koneksi,"SELECT * FROM detail_pesanan
                ...
                WHERE detail_pesanan.karyawan = '$name'
                ORDER BY id_user ASC");
             $index = 1;
?>
        <form action="" method="post" enctype="multipart/form-data">
            <h1 class="display-3 text-center my-4">REKAP PEKERJAAN ANDA DISINI</h1><hr>
            <div class="row">
                <div class="col-md-10">
                    <div class="form-group">
                        <label for="generate">Pilih Nama Anda Disini</label>
                        <select class="form-control" id="generate" name="generate" Required>
                        <option value="">---Pilih Disini---</option>
                        <?php
                            $sql = "SELECT DISTINCT(karyawan) FROM `detail_pesanan`;";
                            $query = $conn->query($sql);
                            while ($dt = $query->fetch_array()) {
                        ?>
                            <option value="<?php echo $dt['karyawan'] ?>" <?php if(isset($_POST['submit'])){if($dt['karyawan']==$_POST['generate']){echo "selected";}} ?>>
                                <?php echo $dt['karyawan'] ?></option>
                        <?php } ?>
                        </select>
                    </div>
                </div>
